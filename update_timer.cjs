...
const newTimerUI = `<div className="flex flex-col items-center justify-center bg-slate-900 px-6 py-2 rounded-xl shadow-inner border border-slate-700">
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Sisa Waktu</div>
...
const newHeader = `<div className="max-w-3xl mx-auto p-3 sm:p-4 flex items-center justify-between w-full bg-slate-800 text-white rounded-b-xl shadow-lg border-b-4 border-slate-900 mb-2">`;
...
const newButton = `<Button variant={isTimerRunning ? "destructive" : "default"} size="lg" className={isTimerRunning ? "w-24 font-bold" : "w-24 font-bold bg-blue-600 hover:bg-blue-500 text-white"} onClick={toggleTimer}>`;
...
// Fix the back button icon color
const backBtnRegex = /<Button variant="ghost" size="icon" onClick=\{[^}]+\} className="shrink-0">/m;
const newBackBtn = code.match(/<Button variant="ghost" size="icon" onClick=\{[^}]+\} className="shrink-0">/m)[0].replace('className="shrink-0"', 'className="shrink-0 text-slate-300 hover:text-white hover:bg-slate-700"');
code = code.replace(backBtnRegex, newBackBtn);
...
const newNameDisplay = `<div className="flex-1 min-w-0 px-2 sm:px-4 text-left">
            <h2 className="font-bold text-white text-lg sm:text-xl truncate drop-shadow-sm">
              {participant?.name || 'Loading...'}
            </h2>
            <div className="flex items-center gap-2 mt-1">
              <span className="bg-slate-700 text-slate-300 text-[10px] sm:text-xs px-2 py-0.5 rounded font-medium">{participant?.category}</span>
              <span className="text-[10px] sm:text-xs text-slate-400">Juri {post}</span>
            </div>
          </div>`;
