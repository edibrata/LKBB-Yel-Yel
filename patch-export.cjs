...
const newExport = `  const exportHasilLomba = async () => {
    try {
      ...
      const baText = \`Pada hari ini \${dayStr} tanggal Sembilan Belas bulan September tahun Dua Ribu Dua Puluh Enam bertempat di Kecamatan Sukaresmi telah dilaksanakan Kegiatan Penjelajahan Pramuka Penggalang Tahun 2026.\\n\\nBerdasarkan kegiatan tersebut diperoleh hasil lomba LKBB dan Yel-Yel sebagaimana terlampir.\\n\\nDemikian Berita Acara ini dibuat untuk diketahui dan digunakan sebagaimana mestinya.\`;
...
// replace from `  const numberToText = (num) => {` to `const exportToXLSX = async () => {`
content = content.replace(
  /const numberToText = \(num\) => \{[\s\S]*?(?=const exportToXLSX = async \(\) => \{)/,
  newExport + '\n\n  '
);
